...
// Rate limit (mirror job-metadata.php): 20/hour per IP.
$ip = $_SERVER['REMOTE_ADDR'] ?? 'unknown';
$rate = sys_get_temp_dir() . '/arc-servicemeta-' . hash('sha256', $ip) . '.json';
...
// A service description must carry at least a name; other fields (description, model,
// inputSpec, outputSpec, priceUSDC) are optional and pinned as-is.
if (!is_array($decoded) || !isset($decoded['name'])) { http_response_code(400); echo json_encode(['error' => 'Invalid service JSON (name required)']); exit; }
// Re-encode canonically so the pinned bytes are stable and the CID / metadata hash is deterministic per content.
...
if ($jwt !== '') {
  $tmp = tempnam(sys_get_temp_dir(), 'arc-service-');
  file_put_contents($tmp, $canonical, LOCK_EX);
  $curl = curl_init('https://uploads.pinata.cloud/v3/files');
  $post = ['file' => new CURLFile($tmp, 'application/json', 'service-metadata.json'), 'network' => 'public', 'name' => 'arcodian-service-' . hash('sha256', $canonical)];
...
$dir = dirname(__DIR__) . '/service-metadata';
...
echo json_encode(['url' => $scheme . '://' . $host . '/service-metadata/' . $hash . '.json?sha256=' . $hash, 'storage' => 'local-fallback']);
