...
export default function LendApp({ account, chainId, activeProvider, connect, disconnect }: Props) {
  ...
  return <main className="lend-site">
    <nav><a href="https://arcodian.fun" className="lend-brand"><img src="/arcodian-mark.svg" alt=""/><span>ARCODIAN<small>ARC LEND</small></span></a><div className="lend-nav-links"><a href="https://arcodian.fun/swap">Product</a><a className="active" href="https://lend.arcodian.fun">Market</a><a href="https://arcodian.fun/arcpay">Pay</a><a href="https://arcodian.fun/agentpay">Agent</a><a href="https://lend.arcodian.fun/admin">Admin</a></div><div className="lend-wallet"><a href="https://wallet.arcodian.fun">Wallet</a><button onClick={account ? disconnect : connect}>{short(account)}</button></div></nav>
    ...
    <section className="lend-terminal">
      ...
      {!account ? <div className="lend-gate"><h2>Connect an EVM wallet</h2><p>MetaMask, Rabby, OKX, Bitget, Coinbase Wallet, or another injected EVM wallet.</p><button onClick={connect}>Connect wallet</button></div> : <>
        ...
        <div className="lend-actions">
          <article><p>BORROW ASSET · USDC</p><h2>Supply & borrow USDC</h2><label>Amount<input inputMode="decimal" value={amount} onChange={e=>setAmount(e.target.value)} placeholder="0.00"/><b>USDC</b></label><div><button disabled={busy||!amount} onClick={()=>action("supply")}>Supply USDC</button><button disabled={busy||!amount||!position.oracleFresh} title={!position.oracleFresh ? "Oracle is stale — borrowing is fail-closed on-chain until it's refreshed" : undefined} onClick={()=>action("borrow")}>Borrow USDC</button><button disabled={busy||!amount} onClick={()=>action("repay")}>Repay USDC</button><button disabled={busy} onClick={()=>action("withdraw")}>Withdraw supply</button></div></article>
          <article><p>EURC COLLATERAL</p><h2>Manage collateral</h2><label>Amount<input inputMode="decimal" value={collateral} onChange={e=>setCollateral(e.target.value)} placeholder="0.00"/><b>EURC</b></label><div><button disabled={busy||!collateral} onClick={()=>action("collateral")}>Approve + deposit</button><button disabled={busy||!collateral||(!position.oracleFresh && Number(position.borrowed) > 0)} title={!position.oracleFresh && Number(position.borrowed) > 0 ? "Oracle is stale — withdrawing collateral against open debt is fail-closed on-chain until it's refreshed" : undefined} onClick={()=>action("withdrawCollateral")}>Withdraw</button></div><small>Deposited: {Number(position.collateral).toFixed(4)} EURC</small></article>
        </div>
      </>}
