...
export default function Analytics(){
 ...
 return <section className="analytics">
  ...
  <div className="analytics-grid">
   ...
   <article><p>BRIDGE · CCTP</p><h2>{money(data.bridge.volumeUsd)}</h2><span>Successful Arc-origin burn calls</span><dl><div><dt>Burns</dt><dd>{data.bridge.burns}</dd></div><div><dt>Completed verified</dt><dd>{data.bridge.completionChecked?data.bridge.completed:"—"}</dd></div><div><dt>Completion coverage</dt><dd>{data.bridge.completionChecked}/{data.bridge.burns}</dd></div></dl><a href="/bridge">Open bridge →</a></article>
   ...
   <article><p>MARKET + DEX</p><h2>{money(data.market.volumeUsd)}</h2><span>Indexed curve and DEX volume</span><dl><div><dt>Launches</dt><dd>{data.market.launches}</dd></div><div><dt>Trades</dt><dd>{data.market.trades}</dd></div><div><dt>Holders</dt><dd>{data.market.holders}</dd></div></dl><a href="/market">Open market →</a></article>
   <article><p>ARC LEND</p><h2>{money(data.lend.suppliedUsd)}</h2><span>Total supplied assets</span><dl><div><dt>Borrowed</dt><dd>{money(data.lend.borrowedUsd)}</dd></div><div><dt>Utilization</dt><dd>{Number(data.lend.utilizationPct).toFixed(2)}%</dd></div><div><dt>Reserve revenue</dt><dd>{money(data.lend.reserveRevenueUsd)}</dd></div></dl><a href="/lend">Open Lend →</a></article>
