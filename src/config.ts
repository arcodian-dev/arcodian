...
// AgentRail — non-custodial prepaid-credit voucher rail (deployed Arc testnet 2026-08-08, block ~55975638).
// Deposit USDC -> allocate per provider -> sign EIP-712 vouchers off-chain -> provider redeems latest
// voucher on-chain, 0.5% fee -> FEE_TREASURY. Service catalog bound to passport agentId.
export const SERVICE_REGISTRY_ADDRESS = "0x26aBc806Ab5feae90DE708F56922f6f19F20F8Db";
export const PAY_VAULT_ADDRESS = "0xD6bC439615D6fc2EA8cD6eC6CAC413E8F7e85dD1";
export const SERVICE_METADATA_ENDPOINT = "https://arcodian.fun/api/service-metadata.php";
export const SERVICES_ENDPOINT = "https://arcodian.fun/developers/services.json";
export const SERVICE_REGISTRY_ABI = ["function registerService(uint256 price,string endpointURI,string metadataURI) returns(bytes32)","function updateService(bytes32 serviceId,uint256 price,string endpointURI,string metadataURI)","function deactivateService(bytes32 serviceId)","function getService(bytes32) view returns((address provider,uint256 agentId,uint256 price,string endpointURI,string metadataURI,bool active))","event ServiceRegistered(bytes32 indexed serviceId,address indexed provider,uint256 indexed agentId,uint256 price,string endpointURI,string metadataURI)","event ServiceUpdated(bytes32 indexed serviceId,uint256 price,string endpointURI,string metadataURI)","event ServiceDeactivated(bytes32 indexed serviceId)"];
export const PAY_VAULT_ABI = ["function deposit() payable","function allocate(address provider,uint256 amount)","function redeem(address payer,uint256 cumulative,bytes signature)","function withdrawFree(uint256 amount)","function requestDeallocate(address provider)","function finalizeDeallocate(address provider)","function freeBalance(address) view returns(uint256)","function subs(address,address) view returns(uint256 allocated,uint256 redeemed,uint64 deallocateAt)","function DOMAIN_SEPARATOR() view returns(bytes32)","event Deposited(address indexed payer,uint256 amount,uint256 freeBalance)","event Allocated(address indexed payer,address indexed provider,uint256 amount,uint256 allocated)","event Redeemed(address indexed provider,address indexed payer,uint256 cumulative,uint256 paid,uint256 fee)"];
// EIP-712 voucher: domain {name:"AgentRail",version:"1",chainId,verifyingContract:PAY_VAULT_ADDRESS};
// type Voucher{address payer,address provider,uint256 cumulative}. cumulative monotonic per (payer,provider).
export const VOUCHER_DOMAIN = { name: "AgentRail", version: "1" };
export const SERVICE_DEPLOY_BLOCK = 55975600;
