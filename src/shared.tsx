...
export const FAQ_ITEMS = [
  ["Is Arcodian live on mainnet?", "Mostly. Bridge (Circle CCTP), the USDC-only Market/Launchpad, and Swap (Arcodian's own on-chain routing — an Arcodian-deployed AMM plus permissionless external Arc Mainnet pools) are all live on Arc Mainnet, chain 5042, moving real USDC — verify any address before signing. Bridging out of Arc works end to end; bridging into Arc currently doesn't (burns land, but Circle's attestation service isn't yet attesting messages destined for Arc — a Circle-side limitation, expected to lift once Arc's public mainnet launches). StableCoin FX (no official Arc Mainnet EURC address exists yet), Arc Lend, EURC launches, and the agent-economy contracts (Passport, Jobs, Reputation, Agent Pay) remain Arc Testnet only, where test USDC and test tokens have no financial value."],
