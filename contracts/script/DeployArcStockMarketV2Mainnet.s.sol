...
/// Arc Mainnet: ArcStockMarket with the 15-minute LP lock, on the existing
/// ArcSignedPriceFeed (same signer, same feed ids). Replaces 0x6171…12C1,
/// which has a 24-hour lock and was never funded; pause that one after this.
contract DeployArcStockMarketV2Mainnet is Script {
    address constant TREASURY = 0xF1CBe360b45F2E22Ab74A2c434e5602f66105CaF;
    ArcSignedPriceFeed constant FEED = ArcSignedPriceFeed(0xE8e3eE1201E6D9dA70B4147CD2E63185A063573d);
    ...
    function run() external returns (ArcStockMarket market) {
        ...
        require(address(FEED).code.length > 0 && FEED.signer() != address(0), "FEED");
        vm.startBroadcast();
        ...
        require(market.LP_LOCK() == 15 minutes, "LOCK");
