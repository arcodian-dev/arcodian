...
contract ArcodianLaunchFactoryV12 is IUnlockCallback {
    using PoolIdLibrary for PoolKey;

    uint8 public constant ENGINE_VERSION = 12;
    ...
    /// Usable tick bounds for a full-range, single-sided position.
    function _range(bool tokenIsZero) private pure returns (int24 lower, int24 upper, int24 start) {
        ...
        lower = min;
        upper = max;
        // currency0's price falls as the tick falls. A token that is
        // currency0 starts at the bottom of the range, where the position is
        // entirely currency0; as currency1 it starts at the top.
        start = tokenIsZero ? min : max;
    }
