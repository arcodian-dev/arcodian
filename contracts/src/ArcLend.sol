    ...
    constructor(
        ...
        if (
            address(collateral_) == address(0) || address(oracle_) == address(0) || admin_ == address(0)
                || guardian_ == address(0) || supplyCap_ == 0 || borrowCap_ == 0 || annualRateWad_ > WAD
        ) revert Invalid();
        ...
        ratePerSecond = annualRateWad_ / 365 days;
...
contract ArcManualOracle is IArcPriceOracle {
    address public immutable admin;
    uint256 public value;
    uint64 public updatedAt;

    constructor(address admin_, uint256 initialPrice) {
        if (admin_ == address(0) || initialPrice == 0) revert();
        admin = admin_;
        value = initialPrice;
        updatedAt = uint64(block.timestamp);
    }

    function setPrice(uint256 next) public {
        if (msg.sender != admin || next == 0) revert();
        value = next;
        updatedAt = uint64(block.timestamp);
    }
