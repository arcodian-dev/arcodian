...
contract ArcodianLaunchFactoryV14Test is Test {
    using PoolIdLibrary for PoolKey;

    PoolManager manager;
    ArcodianV4Router router;
    PoolSwapTest swapper;
    ArcodianLaunchFactoryV14 factory;
    ArcodianLaunchHookV14 hook;
    V14Quote quote;
    ...
    function _deploy(address quoteAt) internal {
        ...
        deployCodeTo("ArcodianLaunchFactoryV14.t.sol:V14Quote", quoteAt);
        quote = V14Quote(quoteAt);
        address flagged = address(FLAGS | (uint160(0x5151) << 144));
        deployCodeTo(
            "ArcodianLaunchHookV14.sol:ArcodianLaunchHookV14",
            abi.encode(manager, address(this), treasury, Currency.wrap(quoteAt)),
            flagged
        );
        hook = ArcodianLaunchHookV14(flagged);
        factory = new ArcodianLaunchFactoryV14(manager, hook, Currency.wrap(quoteAt), treasury);
    ...
    function testNoLaunchBuyNeedsNoQuote() public {
        ...
        assertEq(token.balanceOf(treasury), 10_000_000 ether, "1% launch fee in supply");
        assertEq(factory.ENGINE_VERSION(), 14);
    ...
    function _checkInitialBuy(address quoteAt) internal {
        ...
        assertLt(bought, 10_000_000 ether);
    ...
    function testClaimPaysUsdc() public {
        ...
        vm.prank(creator);
        vm.expectRevert(ArcodianLaunchHookV14.NothingToClaim.selector);
        hook.claim(Currency.wrap(address(quote)));
    }
    ...
    function testFactoryBindsOnce() public {
        _deploy(HIGH);
        vm.expectRevert(ArcodianLaunchHookV14.FactoryAlreadySet.selector);
        hook.setFactory(address(0xBAD));
        vm.prank(address(0xBAD));
        vm.expectRevert(ArcodianLaunchHookV14.NotFactory.selector);
