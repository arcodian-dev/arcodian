...
import { ARC_X402, buildNanopaymentAuthorization, enrichedReceiptStatement, parseX402Challenge, verifyNanopaymentReceipt } from "./nanopayments";
...
const sellerSigner = Wallet.createRandom();
...
describe("Phase E1 x402 primitives", () => {
  ...
  it("builds deterministic unsigned EIP-3009 typed data bound to Passport and request", async () => {
    ...
    expect((a.typedData as any).message.from).toBe(wallet);
    expect(a.policyAmount).toBe("10000000000000000");
    expect((a.typedData as any).message.validBefore).toBe("2000604900");
  ...
  it("verifies an exact receipt and rejects mutation", async () => {
    const key = `0x${"33".repeat(32)}`;
    const proof = { transaction: "batch-1", network: ARC_X402.network, payer: wallet, amount: "10000", payTo: seller, idempotencyKey: key,
      requestHash: `0x${"44".repeat(32)}`, serviceId: "svc", authorizationDigest: `0x${"55".repeat(32)}` };
    const sellerSignature = await sellerSigner.signMessage(enrichedReceiptStatement(proof));
    const response = Buffer.from(JSON.stringify({
      success: true, network: ARC_X402.network, payer: wallet, transaction: "batch-1",
      extensions: { amount: "10000", payTo: seller, idempotencyKey: key, requestHash: proof.requestHash, serviceId: proof.serviceId, authorizationDigest: proof.authorizationDigest },
      sellerSignature,
    })).toString("base64");
    expect((await verifyNanopaymentReceipt({ paymentResponse: response, payer: wallet, idempotencyKey: key, amount: "10000", payTo: seller })).paymentBindingVerified).toBe(true);
    expect((await verifyNanopaymentReceipt({ paymentResponse: response, payer: wallet, idempotencyKey: key, amount: "10001", payTo: seller })).paymentBindingVerified).toBe(false);
    ...
    const limited = await verifyNanopaymentReceipt({ paymentResponse: officialMinimal, payer: wallet, idempotencyKey: key, amount: "10000", payTo: seller });
